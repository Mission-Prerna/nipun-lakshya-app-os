...
class ChatBotActivity : AppCompatActivity() {
    ...
    //Downloads
    private val downloadedFiles: MutableMap<String, String> by lazy { loadDownloadedFiles() }

    private fun loadDownloadedFiles(): MutableMap<String, String> {
        val downloadDirectory = File(
            Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS),
            chatVM.nlFolder
        )
        val downloadedFiles = downloadDirectory.list()
        val downloadedMap = mutableMapOf<String, String>()
        downloadedFiles?.map { fileName ->
            downloadedMap[fileName.substringBeforeLast(".")] = fileName
        }
        Timber.d("loadDownloadedFiles: $downloadedMap")
    ...
    fun isAssetDownloaded(
        ...
        return downloadedFiles.contains(assetId)
    }
    ...
    fun onAssetClicked(
        ...
        if (downloadedFileName != null) {
            openAsset(
                asset = chatVM.getDownloadedFileFromName(
                    context = this,
        ...
        chatVM.botsLiveData.observe(this) { state ->
            ...
            when (state) {
                ...
                is ChatbotListingState.OpenAsset -> {
                    downloadedFiles[state.assetId] = state.assetName
                    openAsset(
    ...
    //Assets
    private fun openAsset(asset: File, type: String) {
        onMainThread {
            binding.progressBar.visibility = View.GONE
            when (type.uppercase()) {
                DownloadableType.IMAGE.name -> triggerOpen(asset, "image/*")
                DownloadableType.VIDEO.name -> triggerOpen(asset, "video/*")
                DownloadableType.DOC.name -> triggerOpen(asset, "application/pdf")
                else -> {
                    Timber.d("openAsset: unknown type")
                }
            }
        }
    }

    private fun triggerOpen(asset: File, type: String) {
        Timber.d("openDoc: opendoc $asset")
        val intent = Intent(Intent.ACTION_VIEW)
        val uri = FileProvider.getUriForFile(
            this,
            "org.samagra.missionprerna.android.provider",
            asset
        )
