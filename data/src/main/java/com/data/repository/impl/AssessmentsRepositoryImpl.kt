...
class AssessmentsRepositoryImpl @Inject constructor(
    ...
    override suspend fun convertStatesToSubmissions(): Result<Unit> {
        val states = assessmentStateDao.getDetailedStates()
        ...
        states.forEach { state ->
            ...
            if (submissionsMap.containsKey(state.studentId).not()) {
                ...
                submitResultsModel.udise = mentorDetails.udise
        ...
        if (submissionCount.isNotEmpty()) {
            clearStates()
            updateHomeScreenStats(assessmentHistories)
        }
        return Result.Success(Unit)
    }

    private suspend fun updateHomeScreenStats(assessmentHistories: List<StudentAssessmentHistory>) {
