                ...
                when (it) {
                    ...
                    is AuthenticationRepository.MentorDataSaved.MentorDataSaveSuccessful -> {
                        ...
                        if (mentor.actorId == TEACHER_ACTOR_ID) {
                            ...
                            udise?.let { udise->
                                val studentsResult = studentsRepository.fetchStudents(udise)
                                val cal = Calendar.getInstance()
                                studentsRepository.fetchStudentsAssessmentHistory(udise = udise, grade = "1,2,3", month = cal.get(Calendar.MONTH) + 1, year = cal.get(Calendar.YEAR))
                                when (studentsResult) {
                                    is Result.Success -> {
                                        mentorDataSavedState.postValue(it)
                                    }
